...
<body>
	...
	<p>
		...
		우리가 기존에 a태그로 요청 및 form submit 요청 방식은 "동기식 요청" 이었음 => 응답페이지가 돌아와야만 볼 수 있음(페이지가 깜빡거림) <br><br>
		
		* 동기식 / 비동기식 <br>
		> 동기식(a, form submit)
		<ul>
	    	<li>요청 처리 후 그에 해당하는 응답페이지가 돌아와야만 그 다음 작업 가능</li>
			<li>서버에 요청한 결과까지의 시간이 지연되면 무작정  계속 기다려야 함(흰 페이지로 보여질거임)</li>
	    ...
	    <ul>
	    	...
	    	<li>요청을 보내놓고 그에 해당하는 응답이 돌아올 때까지 현재 페이지에서 다른 작업을 할 수 있음</li>
	    </ul>
	    ex) 실시간 급상승 검색어 로딩, 검색어 자동완성, 아이디 중복확인, 찜하기 / 해제하기, 추천, 댓글, 무한스크롤링(페이징 대체) 등등..
	    <br><br>
	    
	    * 비동기식의 단점
	    - 현재 페이지에 지속적으로 리소스가 쌓임 => 페이지가 느려질 수 있음 <br>
	    - 에러 발생시 디버깅이 어려움 <br>
	    - 요청 처리 후에 돌아온 응답 데이터(nnnn or nnny)가지고 현재 페이지에 새로운 요소를 만들어서 뿌려줘야됨 => dom요소들을 새로이 만들어내는 구문을 잘 익혀둬야함
	    <br>
	    ...
	    * 주요 속성
	    - url : 요청할 url (필수속성!)
	    - type|method :요청전속방식(get/post)
	    - date : 요청시 전달할 값
	    - success : ajax 통신 성공했을 때 실행할 함수 정의해둘 수 있음
		...
		$(function(){
			$("#btn1").click(function(){
				...
				$.ajax({
					...
					type:"get",
					success:function(result){ // 매개변수에 돌아온 응답데이터 담김
					...
					complete:function(){
						console.log("ajax 통신 여부와 관계 없이 무조건 출력");
					},
	...
	<script>
	 	function test2(){
	 		$.ajax({
	 			...
	 			success:function(a){
	 				$("#output2").text(a);
	 				$("#input2_1").val(""); 성공시 값을 지워 없앰
	 				$("#input2_2").val(""); 성공시 값을 지워 없앰
	 			}, // a말고 다른 거 가능.. 사용자 정의 
	 			error:function(){
	 				console.log("ajax 통신 실패");
	 			},
			...
			$.ajax({
				...
				success : function(result){ // [{key:value, key:value,...},{},{}]
					...
					for(let i=0; i<result.length; i++){
						value += "<tr>" // 누적하고 있기 때문에 value에 쌓이는 중~
									//+ "<td>" + result[i]  이렇게 하면 한 객체
