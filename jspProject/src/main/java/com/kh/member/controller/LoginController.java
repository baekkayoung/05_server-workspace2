...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 1) application : 여기에 담긴 데이터는 "웹 애플리케이션 전역"에서 다 꺼내서 쓸 수 있음 (이 프로젝트에서)
		 * 2) session : 여기에 담긴 데이터는 내가 지우기 전까지, 세션이 만료(서버가 멈추거나, 브라우저 종료)되기 전까지 살아있는 데이터.
		 * 				"어떤 jsp든, 어떤 servlet"이든 꺼내서 쓸 수 있음
		 ...
		 * 	       데이터를 꺼내고자한다면 .getAttribute("키") : Object 타입으로 벨류
		 * 	       데이터를 지우고자한다면 .removeAttribute("키")
		 * 
		 */
		
		if(loginUser == null) {
			// 조회결과 없음 == 로그인 실패화면 => 에러문구가 보여지는 에러페이지() 응답
			request.setAttribute("errorMsg", "로그인에 실패했습니다.");
			// 응답페이지(jsp) 포워딩
			RequestDispatcher view = request.getRequestDispatcher("views/common/errorPage.jsp"); // 응답뷰 생김
...
//			session.
			// 서블릿에서는 session에 접근하고자한다면 우선 세션 객체 얻어와야함!
			HttpSession session = request.getSession();
			session.setAttribute("loginUser", loginUser);
			
			// 1. index 포워딩 방식으로 응답 view를 출력하는 방식
//			RequestDispatcher view =  request.getRequestDispatcher("index.jsp");
//			view.forward(request, response);
			// 해당 선택된 jsp가 보여질 뿐 url에는 여전히 현재 이 서블릿 매핑값이 남아있음
			// localhost:8001/jsp/login.me => 좀 별로임
