...
public class ThumbnailInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (ServletFileUpload.isMultipartContent(request)) {
			...
			String savePath = request.getSession().getServletContext().getRealPath("/resources/thumbnail_upfiles/"); // applicattion에 접근
			...
			Board b = new Board(); // 모든 필드가 초기화된 값으로,
			...
			// Attachment에 여러번 insert를 할 데이터 뽑기
			ArrayList<Attachment> list = new ArrayList<Attachment>();
			
			for(int i=1; i<5; i++) {
				String key = "file" + i;
				
				if(multiRequest.getOriginalFileName(key) != null) { // 그거 원본명이 null이 아님 => 첨부 했다는 뜻
					// 실제로 첨부파일이 존재할 경우
